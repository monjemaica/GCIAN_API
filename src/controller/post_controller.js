    ...
    if (err) {
      ...
      res
        ...
        .send({
          message:
            err.message ||
            `Errors found while retireving student by id: ${req.params.studid_fld}`,
        });
    }
    res.send(data);
  });
    ...
    if (err) {
      ...
      res
        ...
        .send({
          message:
            err.message ||
            `Errors found while retireving student by id: ${req.params.post_uid}`,
        });
    }
    res.send(data);
  });
    ...
    Posts.updateById(req.params.post_uid, post, (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res
            .status(404)
            .send({ message: `Record not found: ${req.params.post_uid}` });
        }
        res.status(500).send({
          message:
            err.message ||
            `Error updating post with post_uid ${req.params.post_uid}`,
        });
      }
      res.send(data);
    });
  ...
  Posts.removeById(req.params.post_uid, post, (err, data) => {
    if (err) {
      ...
      res.status(500).send({
        message:
          err.message ||
          `Could not delete post with post_uid ${req.params.post_uid}`,
      });
    }
    res.send({
      message: `Post ${req.params.post_uid} was deleted successfully`,
      data: data,
    });
  });
};
    ...
    Posts.postLike(req.params.post_uid, (err, data) => {
      if (err) {
        ...
        res.status(500).send({
          message:
            err.message ||
            `Error updating liked post with post_uid ${req.params.post_uid}`,
        });
      }
      res.send(data);
    });
    ...
    Posts.postDislike(req.params.post_uid, (err, data) => {
      if (err) {
        ...
        res.status(500).send({
          message:
            err.message ||
            `Error updating disliked post with post_uid ${req.params.post_uid}`,
        });
      }
      res.send(data);
    });
...
exports.liked = (req, res) => {
  Posts.like(req.params.like_uid, (err, data) => {
    if (err) {
      res.status(500).send({
        message: err.message || "Errors found while liking post",
      });
...
exports.findCCSPosts = (req, res) => {
  
    Posts.getNumPostsCCS((err, data) => {
        if(err){
            if(err.kind === "not_found"){
                res.send([])  
             }
